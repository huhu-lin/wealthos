/**
 * /api/kline-us — Vercel Edge Function
 *
 * Server-side proxy for US stock K-line data from Render.
 * Eliminates browser CORS issues: browser → Vercel (same-origin) → Render (server-to-server).
 *
 * Usage (from frontend):
 *   GET /api/kline-us?ticker=QLD&days=720
 */
...
export default async function handler(req) {
  ...
  const upstreamUrl = `${KLINE_API}/kline/us?ticker=${encodeURIComponent(ticker)}&days=${daysNum}`;
  ...
  for (let attempt = 0; attempt <= 2; attempt++) {
    try {
      ...
      console.warn(`[kline-us] attempt ${attempt + 1} failed: ${err.message}`);
      if (attempt < 2) {
        // 等待 Render 冷啟動（server-to-server 不受瀏覽器 CORS 影響，可以重試）
        await new Promise(r => setTimeout(r, 5000));
      } else {
        console.error("[kline-us] all retries failed:", err.message);
        return new Response(
