...
async function fetchKline(ticker, isUS = false) {
  try {
    ...
    console.log(`[${ticker}] yfinance 還原K線筆數: ${closes.length}, 最新: ${closes.at(-1)?.toFixed(2)}`);
    return closes;
  } catch(e) {
...
/**
 * 掃描整個 closes，旗標跨K棒記憶，與 TradingView Pine 邏輯一致
 * 只回傳「最新一根 K 棒」是否觸發訊號
 */
function checkSignal(closes, jThresholdEntry=10, jThresholdExit=90) {
  ...
  for (let i = 1; i < closes.length; i++) {
    ...
    if (jBelowFlag && kdj.j > jThresholdEntry) {
      signal = i === closes.length-1 ? 'BUY' : null;
      jBelowFlag = false;
    }
...
export default async function handler(req) {
  ...
  for (const st of strategyTickers) {
    ...
    console.log(`[${ticker}] K線筆數: ${closes.length}, 最新: ${closes.at(-1)?.toFixed(2)}`);

    const result = checkSignal(closes, jEntry, jExit);
    const { signal, price, bb, kdj, jBelowFlag, jAboveFlag } = result;

    console.log(`[${ticker}] signal=${signal ?? 'null'}, J=${kdj?.j?.toFixed(1)}, 蓄力=${jBelowFlag}, 過熱=${jAboveFlag}`);
