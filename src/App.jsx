    ...
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      {snapshots.length>0 ? (
        <Card style={{padding:18}}>
          <div style={{fontWeight:600,fontSize:13,marginBottom:14}}>資產 / 負債 / 淨值 趨勢</div>
          <ResponsiveContainer width="100%" height={220}>
            <AreaChart data={snapshots}>
              ...
              <YAxis tick={{fill:C.textMuted,fontSize:10}} tickLine={false} axisLine={false} tickFormatter={v=>`${(v/1000000).toFixed(1)}M`}/>
              <Tooltip {...TT} formatter={(v,n)=>[`NT$${fmtM(v)}`,n==="net"?"淨值":n==="assets"?"總資產":"總負債"]}/>
              <Area type="monotone" dataKey="liabilities" stroke={C.red} strokeWidth={1.5} fill="url(#gliabilities)" dot={false}/>
              <Area type="monotone" dataKey="assets" stroke={C.blue} strokeWidth={1.5} fill="url(#gassets)" dot={false}/>
              <Area type="monotone" dataKey="net" stroke={C.accent} strokeWidth={2.5} fill="url(#gnet)" dot={{fill:C.accent,r:3}}/>
            </AreaChart>
          </ResponsiveContainer>
          <div style={{display:"flex",gap:14,justifyContent:"flex-end",marginTop:6}}>
            {[["淨值",C.accent],["總資產",C.blue],["總負債",C.red]].map(([l,c])=>(
...
function Liabilities({liabilities,reload}) {
  ...
  const openEdit = l => {
    setForm({name:l.name,value:String(l.value||""),monthly:String(l.monthly||""),
