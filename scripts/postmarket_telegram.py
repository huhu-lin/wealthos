...
def generate_summary(mkt, vol, inst, news_grouped):
    ...
    prompt = f"""你是一位台灣資深財經分析師，請根據以下今日台股盤後數據，以條列式整理「盤後重點」，繁體中文。

【使用者投資偏好】市值型/大盤 ETF。分析重心：大盤、ETF、權值股（台積電/鴻海/聯發科/台達電/廣達/富邦金/中信金 等）；個股新聞只在對加權指數有顯著影響時提及。

格式（嚴格遵守）：
• 每條以「• 」開頭，每條不超過 30 字
• 第 1 條：今日台股收盤方向（收漲／收跌／震盪收平）+ 關鍵驅動力
• 第 2～4 條：最重要的訊號，從法人買賣超、成交量、國際盤、新聞中擇重
• 最後 1 條：明日盤前最需關注的事件或風險，以「⚠️ 」開頭
